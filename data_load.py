...
class DigitDataMNIST:
    ...
    def load_digit_mnist(self, test=False, val_data=False):
        ...
        self.X_train = np.asarray(train_data.iloc[:, 1:]).reshape(-1, 1, 28, 28)  # .astype(float);
        self.y_train = np.asarray(train_data.iloc[:, 0])

        # Load test data
        test_data = pd.read_csv(self.test_path)
        self.test_data = np.asarray(test_data.iloc[:,:]).reshape(-1, 1, 28, 28)
